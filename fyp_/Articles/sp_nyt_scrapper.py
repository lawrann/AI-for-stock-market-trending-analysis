...
for i in range(2014,2019):
    ...
    SP_year =  get_articles(str(i),'S%26P')
    SP_all = SP_all + SP_year

keys = SP_all[0].keys()

print(keys)
with open('articles_scrapped\sp_articles\sp_articles_uncleaned3.csv', 'w', newline='', encoding='utf-8') as output_file:
    dict_writer = csv.DictWriter(output_file, keys)
    dict_writer.writeheader()
    dict_writer.writerows(SP_all)

import json
with open('articles_scrapped\sp_articles\sp_articles_uncleaned3.json', 'w') as fp:
    json.dump(SP_all, fp)

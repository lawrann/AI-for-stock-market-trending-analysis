...
if not os.path.exists('articles_cleaned\citi_articles'):
    os.makedirs('articles_cleaned\citi_articles')

filter_texts = ["Citibank", "Citigroup", "Citi ", "Citi's", "Citi group", "Citi bank"]
unwanted_texts = ["citi bike", "citi field"]
# Remove irrelevant sections
filter_sections = ["fashion & style", "arts", "books", "food", "sports", "travel", "the upshot", "blogs", "opinion", "u.s.", "your money"]

def splitter(text):
    term_list = ['#citi', 'nyse:c', '$c', 'citigroup']
    new_string = ''
    if '|' in text:
...
with open('articles_scrapped\citi_articles\citi_articles_uncleaned.json') as json_file:
    ...
    for i in data:
        ...
        for text_index in range(len(filter_texts)):
            ...
            for unwanted in unwanted_texts:
                if (unwanted in i["abstract"].lower()) or (unwanted in i["headline"].lower()) or (unwanted in i["lead_paragraph"].lower()) or (unwanted in i["snippet"].lower()):
                    enter_flag = False
...
with open('articles_cleaned\citi_articles\citi_articles_cleaned.csv', 'w', newline='', encoding='utf-8') as output_file:
    dict_writer = csv.DictWriter(output_file, keys)
    dict_writer.writeheader()
    dict_writer.writerows(filter_article_list)

with open('articles_cleaned\citi_articles\citi_articles_cleaned.json', 'w') as fp:
    json.dump(filter_article_list, fp)


with open('articles_cleaned\citi_articles\citi_articles_cleaned.json') as json_file:
    data = json.load(json_file) 
    count = 1
    filter_article_list = []
    for i in data:
